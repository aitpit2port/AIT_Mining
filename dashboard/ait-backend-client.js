...
function login(){
  ...
  loginFrame.style.cssText='position:absolute;inset:0;width:100vw;height:100vh;border:0;border-radius:0;background:#0b1727;box-shadow:none';
...
async function changePassword(currentPassword,newPassword){const s=state();if(!s)throw new Error('Not signed in.');const r=await call({module:'auth',action:'change_password',token:s.token,currentPassword:currentPassword,newPassword:newPassword});if(!r.ok)throw new Error(r.message||r.error);save(Object.assign({},s,{user:r.user,allowedPages:r.allowedPages||s.allowedPages}));global.dispatchEvent(new CustomEvent('ait:password-changed',{detail:state()}));return r}
...
global.AITBackend={login,logout,changePassword,verify,requirePage,listDatasets,dataset,listFiles,fileLink,download,mapLayers,mapLayerData,state,permission,can,canonicalPage,clearSession:clear,ensureBridge};
})(window);
